...
public:
	...
	Client *get_user_from_user_host(Client *client, std::string const&  user, std::string const& host);
	Client *get_user_from_nickname(std::string const& nickname);
	Client *get_user_from_nickname(Client *authority, std::string const& nickname);
	...
	Chan *as_outsider_get_channel_from_id(Client *authority, ChanId const &cid);
	Chan *get_channel_from_id(Client *authority, const ChanId &cid);
	std::vector<ClientId>	*get_all_users(Client *client);
	Chan *get_channel(std::string const& name);
	Chan *get_channel(Client *query_authority, std::string const& name);
	Chan *as_outsider_get_channel(Client *authority, std::string const& name);
